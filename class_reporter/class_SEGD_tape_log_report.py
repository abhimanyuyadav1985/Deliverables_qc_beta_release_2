...
import datetime
from SEGD_tape_log_config import header_dict,column_dict,start_row,omit_row_list
from configuration import Templates_dir, Report_dir,SEGD_Tape_log_template
import shutil
import win32com.client as win32
import os

class SEGD_tape_log_header(object):
...
class SEGD_tape_log_report(QtCore.QObject):
    ...
    def __init__(self,header,data,file_name,thread_name):
        super(SEGD_tape_log_report, self).__init__()
    ...
    def run(self):
        ...
        src_path = os.path.join(os.getcwd(), Templates_dir,SEGD_Tape_log_template)
        ...
        for set in range(1,int(self.header.no_of_sets) +1 ):
            ...
            for obj in self.data:
                if str(obj.set_no) == str(set):
                    ...
                    ws.Range(str(column_dict["fsp"]) + str(row)).Value = obj.fsp
                    ws.Range(str(column_dict["lsp"]) + str(row)).Value = obj.lsp
